...
export function useAlinWebSocket({
  ...
  useEffect(() => {
    ...
    if (ws.current?.readyState === WebSocket.OPEN && user && myStatus?.trim()) {
  ...
  const connectWS = useCallback(() => {
    ...
    if (!wsUrl) { setWsStatus('DISABLED'); return; }
  ...
  }, [addLog, handleMessage, sendJoinPayload]);
  ...
  // Sync avatar/displayName/province khi thay đổi
  useEffect(() => {
    if (ws.current?.readyState === WebSocket.OPEN && userRef.current) {
  ...
  // Push coordinate updates without reconnecting
  useEffect(() => {
    ...
    socket.send(JSON.stringify({ type: 'MAP_MOVE', payload: { lat: currentPosition[0], lng: currentPosition[1], zoom: 13 } }));
