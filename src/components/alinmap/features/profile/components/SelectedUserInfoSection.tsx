...
interface SelectedUserInfoSectionProps {
    ...
    handleAddFriend: (targetUser?: any) => void;
    handleMessage: () => void;
        ...
        <>
            ...
            <div className="pb-8 space-y-3">
                <div className="grid grid-cols-3 gap-2">
                    {isFriend ? (
                        <button disabled className="min-w-0 flex items-center justify-center gap-1.5 bg-emerald-50 border border-emerald-100 text-emerald-700 py-3 rounded-[20px] font-bold text-[10px] sm:text-xs cursor-not-allowed opacity-90">
                            <UserCheck className="w-4 h-4 shrink-0" />
                            <span className="truncate">Bạn bè</span>
                        </button>
                    ) : isPending ? (
                        <button disabled className="min-w-0 flex items-center justify-center gap-1.5 bg-amber-50 border border-amber-200 text-amber-700 py-3 rounded-[20px] font-bold text-[10px] sm:text-xs cursor-not-allowed opacity-90">
                            <Clock className="w-4 h-4 shrink-0" />
                            <span className="truncate">Đã gửi</span>
                        </button>
                    ) : (
                        <button onClick={() => handleAddFriend()} className="min-w-0 flex items-center justify-center gap-1.5 bg-blue-600 hover:bg-blue-500 text-white py-3 rounded-[20px] font-bold text-[10px] sm:text-xs shadow-lg shadow-blue-600/20 active:scale-95 transition-all">
                            <UserPlus className="w-4 h-4 shrink-0" />
                            <span className="truncate">Kết bạn</span>
                        </button>
                    )}

                    <button onClick={handleMessage} className="min-w-0 flex items-center justify-center gap-1.5 bg-gray-100 hover:bg-gray-200 text-gray-900 py-3 rounded-[20px] font-bold text-[10px] sm:text-xs active:scale-95 transition-all shadow-sm">
                        <MessageCircle className="w-4 h-4 shrink-0" />
                        <span className="truncate">Nhắn tin</span>
                    </button>

                    <button onClick={() => { onLocateUser(selectedUser.lat, selectedUser.lng); }} className="min-w-0 flex items-center justify-center gap-1.5 bg-gray-100 hover:bg-gray-200 text-blue-600 py-3 rounded-[20px] font-bold text-[10px] sm:text-xs active:scale-95 transition-all shadow-sm">
